  ...
  return (
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 p-2 sm:p-4">
        <div className="bg-white rounded-lg w-full max-w-2xl max-h-[95vh] overflow-y-auto relative border-b-4 border-black-500">
          {/* Close button */}
          <button
            onClick={onClose}
            className="absolute top-3 right-3 sm:top-4 sm:right-4 text-gray-400 hover:text-gray-500 z-10 bg-white rounded-full p-1 shadow-sm"
            disabled={isSaving}
          ...
          <div className="p-4 sm:p-6">
            ...
            {(job.id === "0" || job.title === "Position no longer available") ? (
              <div className="text-center py-8 sm:py-12">
                <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-3 sm:mb-4">This job posting is no longer available.</h2>
                <p className="text-gray-600 text-sm sm:text-base">The job you are trying to view may have been removed or is no longer accessible.</p>
              </div>
            ) : (
              <>
                {/* Job header */}
                <div className="mb-4 sm:mb-6">
                  ...
                  <div className="mt-3 sm:mt-4 flex flex-wrap gap-2">
                    <span className="px-2 py-1 sm:px-3 sm:py-1 bg-gray-100 rounded-full text-xs sm:text-sm font-medium">
                      {job.salary}
                    </span>
                    <span className="px-2 py-1 sm:px-3 sm:py-1 bg-gray-100 rounded-full text-xs sm:text-sm font-medium">
                ...
                <div className="border-t border-b border-gray-200 py-3 sm:py-4 my-3 sm:my-4">
                  <div className="flex flex-wrap items-center gap-2">
                    <button
                      className="flex-1 min-w-[120px] py-2 sm:py-3 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors disabled:opacity-50 text-sm sm:text-base"
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 sm:gap-2 px-3 py-2 sm:px-4 sm:py-3 bg-gray-600 text-white rounded-lg font-medium hover:bg-gray-700 transition-colors text-sm sm:text-base"
                    >
                      <FaComments className="h-3 w-3 sm:h-4 sm:w-4" />
                    ...
                    >
                      {isSaved ? (
                        <FaStar className="h-4 w-4 sm:h-5 sm:w-5 text-yellow-400" />
                      ) : (
                        <FaRegStar className="h-4 w-4 sm:h-5 sm:w-5 text-gray-500" />
                      )}
                    </button>
                  </div>
                </div>
                ...
                <div className="mb-4 sm:mb-6">
                  ...
                  {job.job_description && (
                    <div className="prose prose-sm max-w-none text-gray-700 overflow-hidden break-words">
                      {job.job_description}
                    </div>
                  )}
                ...
                {Array.isArray(job.skills) && job.skills.length > 0 ? (
                  <div className="mb-4 sm:mb-6">
                    <h4 className="text-base sm:text-lg font-semibold text-gray-800 mb-2">Skills</h4>
                    <div className="flex flex-wrap gap-2">
                      {job.skills.map((skillItem, index) => (
                        <span
                          key={index}
                          className="px-2 py-1 sm:px-3 sm:py-1 bg-gray-100 rounded-full text-xs sm:text-sm font-medium"
                ...
                <div className="border-t border-gray-200 pt-3 sm:pt-4">
                  <div className="flex items-center">
                    {jobWithHR.hrPhoto ? (
                      ...
                      <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gray-300 mr-3 flex items-center justify-center">
