...
const EmployerDashboardPage = () => {
  ...
  const handleDeleteJob = async (jobId: number) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(responseData.message || 'Failed to delete job posting');
  ...
  useEffect(() => {
    if (!loading) {
      ...
      if (user.user_type === "pwd" || user.user_type === "indigenous" || user.user_type === "general") {
        navigate("/ApplicantDashboard", { replace: true });
        return;
      }
  ...
  const filteredApplicants = applicants.filter((applicant) => {
    ...
    const matchesPosition = positionFilter === 'All Positions' || applicant.position === positionFilter;
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        if (!user.id || !token) throw new Error("Please login to access this page");
  ...
  const handleUpdateJobPosting = async (data: {
    ...
    try {
      ...
      const responseData = await response.json();
      if (!response.ok) throw new Error(responseData.message || 'Failed to update job posting');
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="border-b border-gray-200">
            <nav className="-mb-px flex space-x-8">
              ...
              <button onClick={() => setActiveTab("positions")} className={`${activeTab === "positions" ? "border-blue-500 text-blue-600" : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm`}>
                <FaBriefcase className="inline mr-2" />Job Positions
              </button>
              <button onClick={() => setActiveTab("applicants")} className={`${activeTab === "applicants" ? "border-blue-500 text-blue-600" : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm`}>
                <FaUserTie className="inline mr-2" />Applicants
              </button>
            </nav>
          </div>
          <div className="py-6">
              ...
              <div>
                        ...
                        <div className="flex items-center">
                          ...
                          <div className="ml-5 w-0 flex-1">
                            ...
                            <div className="flex items-baseline">
                              ...
                              <div className={`ml-2 flex items-baseline text-sm font-semibold ${metric.trend === "up" ? "text-green-600" : "text-red-600"}`}>{metric.change}</div>
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <div className="bg-white shadow overflow-hidden sm:rounded-lg mb-8">
                  <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
                    <h3 className="text-lg leading-6 font-medium text-gray-900">Recent Hiring Activity</h3>
                  </div>
                        ...
                        <li key={applicant.id}>
                          <div className="px-4 py-4 sm:px-6">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center">
                                <p className="text-sm font-medium text-blue-600 truncate">{applicant.name}</p>
                                <p className="ml-2 flex-shrink-0 text-xs text-gray-500">for {applicant.position}</p>
                              </div>
                              <div className="ml-2 flex-shrink-0 flex">
                                {applicant.status === "hired" && (<span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Hired</span>)}
                                {applicant.status === "interview" && (<span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">Interview</span>)}
                                {applicant.status === "pending" && (<span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">Pending</span>)}
                                {applicant.status === "rejected" && (<span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">Rejected</span>)}
                              </div>
                            </div>
                            <div className="mt-2 sm:flex sm:justify-between">
                              <div className="sm:flex">
                                <p className="flex items-center text-sm text-gray-500">
                                  <FaBriefcase className="flex-shrink-0 mr-1.5 h-4 w-4 text-gray-400" />{applicant.experience} experience
                                </p>
                                <p className="mt-2 flex items-center text-sm text-gray-500 sm:mt-0 sm:ml-6">
                                  <FaCalendarAlt className="flex-shrink-0 mr-1.5 h-4 w-4 text-gray-400" />Applied on {applicant.applied}
                                </p>
                              </div>
                              <div className="mt-2 flex items-center text-sm text-gray-500 sm:mt-0">
                                {applicant.skills.map((skill, i) => (<span key={i} className="mr-2 px-2 py-1 text-xs font-medium bg-gray-100 rounded-full">{skill}</span>))}
                              </div>
                            </div>
                          </div>
                        </li>
                        ...
                        {jobPostings.map((job) => (
                          <li key={job.id}>
                            <div className="px-4 py-4 sm:px-6">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center">
                                  <p className="text-lg font-medium text-blue-600 truncate">{job.job_title}</p>
                                ...
                                <div className="ml-2 flex-shrink-0 flex">
                                  {job.status === "active" ? (
                                    ...
                                    <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">Closed</span>
                                  )}
                                </div>
                              </div>
                              <div className="mt-2 sm:flex sm:justify-between">
              ...
              <div>
                ...
                {isLoadingApplicants ? (
                  ...
                  <div className="bg-white shadow overflow-hidden sm:rounded-lg p-6 text-center">
                    ...
                    <p className="mt-1 text-sm text-gray-500">You haven't received any applications yet.</p>
                  </div>
                ) : (
                  <div className="bg-white shadow overflow-hidden sm:rounded-lg">
                    <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
                          ...
                          <div className="px-4 py-4 sm:px-6">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center">
                                ...
                                <div className="ml-4">
                                  <p className="text-sm font-medium text-blue-600">{applicant.name}</p>
                              ...
                              <div className="ml-2 flex-shrink-0 flex">
                                {applicant.status === "hired" && (<span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Hired</span>)}
                                {applicant.status === "interview" && (<span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">Interview</span>)}
                                {applicant.status === "review" && (<span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">Under Review</span>)}
                                {applicant.status === "rejected" && (<span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">Rejected</span>)}
                                {applicant.status === "pending" && (<span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">Pending</span>)}
                              </div>
                            </div>
                            <div className="mt-2 sm:flex sm:justify-between">
                              <div className="sm:flex">
